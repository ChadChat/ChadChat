...
void io_client_close(io_client_t* client) {
    ...
    if (client == client_list->tail) {
        ...
        client_list->tail->next = client->next;
    }
...
bool io_loop_init(io_loop_t* loop, int port) {
    ...
    // prepare client list with values
    io_client_t* head = &loop->client_list.head;
    head->last = head->next = NULL;
    loop->client_list.tail = head;
...
void io_loop_iter_clients(io_loop_t* loop, io_client_iter_cb callback, void* state) {
    ...
    for (io_client_t* client = loop->client_list.head.next; client != NULL; client = client->next)
        ...
        while (num_events--) {
            ...
            // handle incoming clients on the server
                ...
                while (true) {
                    ...
                    client->last = loop->client_list.tail->last;
                    loop->client_list.tail = loop->client_list.tail->next = client;
